...
class Calculation:
    ...
    def calculate(self, file_status: str, month: str) -> None:
        for folder in os.listdir(self.main_dir):
            static_file = load_workbook(f'{self.main_dir}/{folder}/static.xlsx')[str(datetime.now().year)]
            svod = load_workbook(f'{self.main_dir}/{folder}/{str(datetime.now().year)}/SVOD{str(datetime.now().year)}{file_status}.xlsx')[month]
